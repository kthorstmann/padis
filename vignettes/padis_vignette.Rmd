...
The result is a list with data frames with the same length as `data_list_to`, which has to be a list of data frames as well
...
data_list <- merge_multiple_df(data_long_1, id_var="PAR.ID", list(data_long_2, data_long_3), merge_down = TRUE)
purrr::map(data_list, head)
## merge up, i. e. make the longer data frame short first and merge then
data_list <- merge_multiple_df(data_long_1, id_var="PAR.ID", list(data_short_2, data_short_3), merge_down = TRUE)
purrr::map(data_list, head)
...
selected_variables <- padis::select_vars(data=data, ids="PAR.ID", prefix="the_prefix_of_the_variables")
